...
export class AccountListComponent implements OnInit {
  ...
  getActiveAccountsCount(): number {
    return this.filteredAccounts.filter(account => account.status === 'ACTIVE').length;
  }
  ...
  getAccountTypeClass(type: string | null): string {
    return type?.toLowerCase() || 'current';
  }

  getAccountTypeIcon(type: string | null): string {
  ...
  copyToClipboard(text: string): void {
    navigator.clipboard.writeText(text).then(() => {
      this.snackBar.open('Account ID copied to clipboard', 'Close', {
        duration: 2000,
